...
{
    private double eps = Constants.CALCULATION_ACCURACY;
    ...
    public Triangle(double edgeA, double edgeB, double edgeC)
    {
        ...
        if ((perimeter - maxEdge) - maxEdge < Constants.CALCULATION_ACCURACY)
    ...
    private bool GetIsRightTriangle()
    {
        ...
        if (b - maxEdge > Constants.CALCULATION_ACCURACY)
            Utils.Utils.Swap(ref maxEdge, ref b);

        if (c - maxEdge > Constants.CALCULATION_ACCURACY)
            Utils.Utils.Swap(ref maxEdge, ref c);

        return Math.Abs(Math.Pow(maxEdge, 2) - Math.Pow(b, 2) - Math.Pow(c, 2)) < Constants.CALCULATION_ACCURACY;
    }
